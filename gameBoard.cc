...
static bool touchesBorderAt(const Entity &e, Position base, int top, int left, int bottom, int right){
...
static bool pixelsOverlapAt(const Entity &a, Position posA, const Entity &b, Position posB){
    const auto &pa = a.shape().pixels();
    const auto &pb = b.shape().pixels();
...
// Check: if you undo the movement, will the collision still occur?
static void applyBounce(Entity &self, const Entity &other, Position selfPrev,
                        Position selfCurr, Position otherPrev, Position otherCurr){
    // Geometry axis revolution
    ...
    bool bounceRow = !stillHitIfUndoRow; // if undoing row fixes overlap then row axis caused collision
    bool bounceCol = !stillHitIfUndoCol; // if undoing col fixes overlap then col axis caused collision
    ...
    if(bounceRow == bounceCol){
        ...
        else { bounceRow = true; bounceCol = true; } // corner tie
    }

    // Apply to movement components
    for(auto &m:self.movementComponents()){
        if(auto sm = dynamic_cast<StraightMovement*>(m.get())){
            ...
            if((bounceRow && gm->affectsRow()) || (bounceCol && gm->affectsCol()))
                gm->bounce(3); // reverse direction for 3 ticks and then continue in the og dir
        }
    }
}

void applyBorderBounce(Entity &e, Position prev, Position curr, int top, int left, int bottom, int right){
    ...
    for(auto &m:e.movementComponents()){
        if(auto sm = dynamic_cast<StraightMovement*>(m.get())){
            ...
            if(bounceRow || bounceCol)
                gm->bounce(3); // Bounce for 3 ticks
        }
    }
}
...
void GameBoard::tick(GameState &state){
    ...
    // Apply movements and entity logic
    for(auto &e:ents){
    ...
    for(std::size_t i = 0; i < n; ++i){
        ...
        for(std::size_t j = i + 1; j < n; ++j){
            ...
            if(pixelsOverlap(a, b)){
                // Save next move before correction
    ...
    ents.erase(std::remove_if(
            ents.begin(), ents.end(),
            [](const std::unique_ptr<Entity> &p){ // I freakin HATE this lambda!
